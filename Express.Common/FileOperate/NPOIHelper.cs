...
namespace Express.Common.FileOperate
{
    public class ExcelNpoiHelper
    {
        ...
        /// <summary>
        /// DataTable导出到Excel的MemoryStream,调用后请主动关闭MS
        /// </summary>
        ...
        public static MemoryStream ExportDt(DataTable dtSource, string strHeaderText)
        {
...

            {
                ...
                dsi.Company = "WAES Inc.";
                workbook.DocumentSummaryInformation = dsi;

                SummaryInformation si = PropertySetFactory.CreateSummaryInformation();
                si.Author = "WAES Common"; //填加xls文件作者信息
                si.ApplicationName = "WAES"; //填加xls文件创建程序信息
                si.LastAuthor = "WAES"; //填加xls文件最后保存者信息
                si.Comments = "WAES"; //填加xls文件作者信息
                si.Title = "WAES Export"; //填加xls文件标题信息
                si.Subject = "WAES Export"; //填加文件主题信息
        ...
        public static MemoryStream ExportDt(DataSet dataSet, string strHeaderText)
        {
            HSSFWorkbook workbook = new HSSFWorkbook();
            #region 右击文件 属性信息
            ...
            #endregion
            foreach (DataTable dtSource in dataSet.Tables)
            {
                Random r = new Random(500); ;
                string sheetName = r.Next().ToString();
        ...
        public static void ExportDTtoExcel(DataTable dtSource, string strHeaderText, HttpResponse Response)
        {
            using (MemoryStream ms = ExportDt(dtSource, strHeaderText))
            {
                Response.Clear();
                ...
                //Response.AddHeader("Content-Disposition", "attachment;FileName="+ strHeaderText + ".xlsx"); //如果需要下载而不是在IE中看的话
                Response.AddHeader("Content-Disposition", "attachment;FileName=" + HttpUtility.UrlEncode(strHeaderText+".xls"));
        ...
        public static void ExportDTtoExcel(DataSet dtSource, string strFileName, HttpResponse Response)
        {
            using (MemoryStream ms = ExportDt(dtSource, strFileName))
            {
                Response.Clear();
                Response.ContentType = "application/xls";
                Response.ContentEncoding = System.Text.Encoding.UTF8;
                ...
                for (int i = (headerRowIndex + 1); i <= sheet.LastRowNum; i++)
                {
                    try
                    {
                        ...
                        else
                        {
                            ...
                            if(row.GetCell(0).StringCellValue.Trim() == "" || row.GetCell(0)==null)
                            {
                                break;
                            }
                        }
